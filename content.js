  ...
  class VideoDownloaderContent {
    ...
    async downloadFromVideoStream(videoElement, video) {
      try {
        ...
        if (!stream || stream.getTracks().length === 0) {
          throw new Error(
            "Failed to capture video stream - no tracks available"
          );
        }
        ...
        console.log(
          `Captured stream with ${videoTracks.length} video track(s)`
        );
    ...
    async downloadFromVideoCanvas(videoElement, video) {
      try {
        ...
        canvas.width =
          videoElement.videoWidth || videoElement.clientWidth || 640;
        canvas.height =
